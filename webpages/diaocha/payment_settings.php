							...
							<div id="ps_box_payment_options" class="ps_box_main gradient_red">
								...
								<div class="ps_box_content">
									...
									<div id="ps_optional_settings">
										...
										<div class="paypal_option stripe_option" <?php if(!in_array($payment_properties->merchant_type,array('stripe','paypal_standard'))){ echo 'style="display: none"'; } ?>>
										...
										<div class="paypal_option stripe_option" <?php if(!in_array($payment_properties->merchant_type,array('stripe','paypal_standard'))){ echo 'style="display: none"'; } ?>>
											...
											<div id="ps_recurring_div" <?php if(empty($payment_properties->enable_recurring)){ echo 'style="display: none"'; } ?>>
												...
												<select id="ps_recurring_cycle_unit" <?php if($payment_properties->merchant_type != 'paypal_standard'){ echo 'style="display: none"'; }; ?>>
										...
										<div style="clear: both;margin-top: 10px"></div>
										<div class="paypal_option stripe_option" id="ps_trial_div_container" <?php if(empty($payment_properties->enable_recurring)){ echo 'style="display: none"'; } ?>>
										...
										<!--
										<span class="ps_options_span stripe_option" <?php if($payment_properties->merchant_type == 'stripe'){ echo 'style="display: block"'; } ?>>
											<input id="ps_enable_invoice" <?php if(!empty($payment_properties->enable_invoice)){ echo 'checked="checked"'; } ?> class="checkbox" value="" type="checkbox" style="margin-left: 0px">
											<label class="choice" for="ps_enable_invoice">Send Invoice to User</label>
											<img class="helpmsg" src="images/icons/68_red.png" style="vertical-align: top" title="Send the payment receipt to your user upon completed payment. Your form need to have an email field to use this option. The payment receipt will be sent to the email address being entered by the user into the email field."/>
											
											<div id="ps_enable_invoice_div" <?php if(!empty($payment_properties->enable_invoice)){ echo 'style="display: block"'; }else{ echo 'style="display: none"'; } ?>>
												<label class="description" for="ps_invoice_email" style="margin-top: 5px">User Email Address:</label>
												<select name="ps_invoice_email" id="ps_invoice_email" class="element select large"> 
														<?php
															
															if(!empty($email_fields)){
																foreach ($email_fields as $data){
																	if($payment_properties->invoice_email == $data['value']){
										...
										<span id="ask_billing_span" class="ps_options_span stripe_option" <?php if($payment_properties->merchant_type == 'stripe'){ echo 'style="display: block"'; } ?>>
										...
										<span id="ask_shipping_span" class="ps_options_span stripe_option" <?php if($payment_properties->merchant_type == 'stripe'){ echo 'style="display: block"'; } ?>>
