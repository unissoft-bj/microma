 ...
 ********************************************************************************/
	function mf_process_form($dbh,$input){
		...
		//at this moment, delay notification only available for stripe
		if(($form_properties['payment_enable_merchant'] == 1) && !empty($form_properties['payment_delay_notifications']) && in_array($form_properties['payment_merchant_type'], array('stripe'))){
	...
	function mf_get_merchant_redirect_url($dbh,$form_id,$entry_id){
		...
		if(!empty($payment_enable_merchant)){ //if merchant is enabled
				...
				if($payment_merchant_type == 'paypal_standard'){
					...
					$paypal_params['rm'] 			= 2; //the buyer鈥檚 browser is redirected to the return URL by using the POST method, and all payment variables are included
	...
	//currently this is only being used for stripe, not paypal
