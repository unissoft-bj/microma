 ...
 ********************************************************************************/
	...
	$query = "delete from `".MF_TABLE_PREFIX."column_preferences` where form_id=? and user_id=?";
	$params = array($form_id,$user_id);
	mf_do_query($query,$params,$dbh);

	//save the new preference
	$query = "insert into `".MF_TABLE_PREFIX."column_preferences`(form_id,user_id,element_name,position) values(?,?,?,?)";
	...
	if(!empty($column_preferences)){
		foreach($column_preferences as $data){
			...
			$params = array($form_id,$user_id,$column_name,$position);
			mf_do_query($query,$params,$dbh);
