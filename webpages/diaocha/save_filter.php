 ...
 ********************************************************************************/
	...
	$query = "delete from `".MF_TABLE_PREFIX."form_filters` where form_id=? and user_id=?";
	$params = array($form_id,$user_id);
	mf_do_query($query,$params,$dbh);
	
	//save the new filters
	$query = "insert into `".MF_TABLE_PREFIX."form_filters`(form_id,user_id,element_name,filter_condition,filter_keyword) values(?,?,?,?,?)";

	foreach($filter_properties_array as $data){
		$params = array($form_id,$user_id,$data['element_name'],$data['condition'],$data['keyword']);
		mf_do_query($query,$params,$dbh);
	}
