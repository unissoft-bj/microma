...
if ($_GET['action']=='delete')
{
	...
	$sql="select cid from news where cid=".$id;
	if ($db->rc($sql)>0) {
		msg("有文章属于该分类，请先删除分类下的文章再进行删除操作");
		
	}
 	$sqldelete="delete from newssort where id=".$id;
 	//echo $sqldelete;
	if($db->q($sqldelete)){
...
<div class="head">
                ...
                <div class="sect">
					...
					<?php 
					$sql="select * from newssort where 1=1 $sqladd order by `order` desc";
					$sqlc="select count(id) as c from newssort where 1=1 $sqladd ";
					  //echo $sql;
					$counts_r = $db->r($sqlc);
					$counts = $counts_r[c];
