...
if($_POST){
	...
	if($db->q($sql))
	{
		...
		msg("修改商品成功","index.php",1);
	}
	
} 
 
if (!isset($id)) msg("系统错误！");
$sql="select * from ma_product where id=".$id;
//echo $sql;
$rs=$db->r($sql);
//dump($rs);
?>
...
<div class="head">
                ...
                <div class="sect">
					...
					<div class="field">
						<label>市场价</label>
						<input type="text" size="30" name="pro_price" id="team-create-price" class="f-input" value="<?php echo $rs[price]?>" datatype="require" require="true" />
					</div>
					<div class="field">
						<label>积分价</label>
						<input type="text" size="30" name="pro_jifen" id="team-create-jifen" class="f-input" value="<?php echo $rs[jifen]?>" datatype="require" require="true" />
					</div>
					<div class="field">
						<label>是否上架</label>
						<input type="text" size="30" name="pro_isok" id="team-create-isok" class="f-input" value="<?php echo $rs[isok]?>" datatype="require" require="true" />
					</div>
