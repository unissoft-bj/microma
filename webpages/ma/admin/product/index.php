...
if ($_GET['action']=='delete')
{
	...
	//}
 	$sqldelete="delete from ma_product where id=".$id;
 	//echo $sqldelete;
	if($db->q($sqldelete)){
		msg("删除商品成功","index.php");
	}
}
...
<div class="head">
                    ...
                    <ul class="filter">
                    	<li>
	                    	<form action="" method="get">
	                    	    ...
	                    	    <a href="<?php echo $cfg['siteurl'];?>admin/product/add.php">添加商品</a>                    	
		                    	商品名称：<input type="text" name="title" class="h-input" style="width:90px" value="<?php echo $title?>" >
                ...
                <div class="sect">
					...
					<?php 
					$sql="select * from ma_product where isok=1 $sqladd order by id desc";
					$sqlc="select count(id) as c from ma_product where isok=1 $sqladd ";
					  //echo $sql;
					$counts_r = $db->r($sqlc);
					$counts = $counts_r[c];
					...
					<tr <?php if ($i % 2 ==1 ) echo "class=\"alt\" ";?> id="team-list-id-<?php echo $rs["id"];?>">
						...
						<td><?php echo $rs["jifen"];?>  </td>
						<td><?php echo $rs["isok"];?>  </td>
						...
						<?php 
						if ($rs["cid"]==60){?>
                        <a href="<?php echo $cfg['siteurl'];?>admin/product/newsedit.php?id=<?php echo $rs['id'];?>">编辑</a>
                        <?php }else{ ?>              
						<a href="<?php echo $cfg['siteurl'];?>admin/product/edit.php?id=<?php echo  $rs["id"];?>">编辑</a><?php }?>｜ 
						<a href="<?php echo $cfg['siteurl'];?>admin/product/index.php?action=delete&id=<?php echo  $rs["id"];?>" class="ajaxlink" ask="确定删除该用户吗？">删除</a> </td>
					</tr>
					<?php }?>
