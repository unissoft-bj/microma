...
if($_POST){
	...
	$sql="insert into ma_product (title,cid,picurl,content,des,creattime,price,jifen,isok) 
	values ('$pro_title','$pro_type','$pro_image','$pro_summary','$pro_systemreview','$pro_creattime','$pro_price','$pro_jifen','$pro_isok')";
	//echo $sql;
	if($db->q($sql))
	{
		msg("添加商品成功","index.php",1);
	}
...
<html xmlns="http://www.w3.org/1999/xhtml" id=""><head>
  ...
  <script>
        ...
        function check()
    	{
    		...
    		var i = document.getElementById("jifen").value.length;
    		if(i == 0)//内容不为空
    		{
    			alert("商品积分价不能为空");
    			return false;
...
<div class="head">
                ...
                <div class="sect">
					...
					<div class="field">
						<label>市场价</label>
						<input type="text" id="price" size="30" name="pro_price" id="team-create-news" class="f-input" value="" datatype="require" require="true" />
					</div>
					<div class="field">
						<label>积分价</label>
						<input type="text" id="jifen" size="30" name="pro_jifen" id="team-create-news" class="f-input" value="" datatype="require" require="true" />
					</div>
					<div class="field">
						<label>是否上架</label>
						<input type="text" id="isok" size="30" name="pro_isok" id="team-create-news" class="f-input" value="1" datatype="require" require="true" />
					</div>					 
