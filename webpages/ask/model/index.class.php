...
class index_controller extends common{
	...
	function content_action()
	{
		...
		$show[0]['content'] = html_entity_decode($show[0]['content'],ENT_QUOTES,"GB2312");
	...
	function for_comment_action()
	{
		...
		$data['content']=iconv('UTF-8', 'GB2312', $_POST['content']);
		$data['uid']=$this->uid;
		$data['add_time']=time();
	...
	function topic_action()
	{
		...
		if($_GET['pid']=='')
		{
			$q_class=$this->obj->DB_select_once("q_class","`pid`='0' order by `sort`","id");
			$this->yunset("pid",$q_class['id']);
		}else{
