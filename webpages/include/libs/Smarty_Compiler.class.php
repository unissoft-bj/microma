...
class Smarty_Compiler extends Smarty
{
    ...
    function _compile_tag($template_tag)
    {
        ...
        switch ($tag_command)
		{
			...
			case '/nav':
				...
				break;
			case 'rating':
			 	$this->_push_tag('rating');
				return $this->_complie_rating_start($tag_args);
				break;
			case '/rating':
				$this->_pop_tag('rating');
				return "<?php endforeach; endif; unset(\$_from); ?>";
				break;
			case 'articleclass':
			...
			case '/qlist':
				...
				break;
			case 'qclass':
			 	$this->_push_tag('qclass');
				return $this->_complie_qclass_start($tag_args);
				break;
			case '/qclass':
				$this->_pop_tag('qclass');
				return "<?php endforeach; endif; unset(\$_from); ?>";
				break;
			case 'mlist':
	...
	function lt_navcalss($menu,$classname)
	{
		if($_GET['c'])
		{
			$navact = "c=".$_GET['c'];
	...
	function _complie_fairs_start($tag_args)
	{
 	...
 	function _complie_articleclass_start($tag_args)
	 {
		...
		if(is_array($group) )
		{
			foreach($group as $key=>$value)
			{
				...
				if($paramer[urlstatic]=="1")
				{
					$group[$key][url] = $config['sy_weburl']."/news/".$value[id]."/";
				}else{
					 $group[$key][url] = $this->Url("index","list",array("nid"=>$value[id]),"1");
				}
				if($paramer[arcpic])
				{
	 ...
	 function _complie_userlist_start($tag_args)
	 {
		...
		$where = "a.status<>'2' and a.`r_status`<>'2' and b.`job_classid`<>'' and b.`open`='1'";
		...
		if($paramer['typeids'])
		{
			$where .= " AND b.`type` in (".$paramer['typeids'].")";
	 ...
	 function _complie_key_start($tag_args)
	 {
		...
		if($paramer['iswap'])
		{
			$config['sy_weburl'] = $config['sy_weburl']."/wap";
		}else{
			$index =1;
		}
		if(is_array($keyword))
	...
	function _complie_tiny_start($tag_args)
	 ...
	 function _complie_comlist_start($tag_args)
	 {
		...
		if($paramer['cert'])
		{
			$cert = @explode(",",$paramer['cert']);
			if(is_array($cert))
			{
				foreach($cert as $key=>$value)
				{
					$where .= " AND FIND_IN_SET('".$value."',cert)";
				}
			}
		}
		if($paramer['uptime'])
		...
		if(is_array($ListId))
		{
			...
			foreach($ComList as $key=>$value)
			{
				...
				if(is_array($JobList))
				{
					...
					$ComList[$key]["job"].="</div><div class=\"area_right\"><a href='".$com_url."'>".$value["username"]."</a>".$value["beizhu"]."</div><div class=\"area_left_bot\"><a href='".$jobs_url."'>全部职位</a></div><div class='area_right_bot'><a href='".$com_url."'>公司详情</a></div>";
	 ...
	 function _complie_article_start($tag_args)
	 {
		...
		$where=1;
		if($_SESSION['did']){
			$where.=" and `did` in(0,".$_SESSION['did'].")";
		}else{
			$where.=" and `did`='0'";
		}
		if(is_array($paramer))
	  ...
	  function _complie_rating_start($tag_args)
	 {
		$paramer = $this->_parse_attrs($tag_args);
		$item = str_replace("'","",$paramer[item]);
		$time=time();
		...
		$List=$db->select_all("company_rating",$where.$limit);
		$this->_tpl_vars[$item] = $List;
		$tag_args = "from=\${$item} " . $tag_args;
		return $this->_compile_foreach_start($tag_args);
	 }
	 ...
	 function _complie_link_start($tag_args)
	 {
		...
		if(is_array($link))
		{
			...
			{
				if($value['domain']!='0' && $value['domain']!=$domain)
				{
					continue;
		 ...
		 function _complie_comjob_start($tag_args)
	 {
		...
		if($paramer['cityid'])
		{
			$where.=" AND `job1_son`='".$paramer['cityid']."'";
		}
		if($paramer['three_cityid'])
		{
			$where.=" AND `job_post`='".$paramer['three_cityid']."'";
		}
		if($paramer['rec'])
		{
		...
		if(!empty($uids))
		{
			...
			foreach($uids as $key=>$value){
				...
				$time = time() - $lasttime[$value];
				if($time>86400 && $time<604800){
					$ComList[$key]['lasttime'] = ceil($time/86400)."天前";
				}elseif($time>3600 && $time<86400){
					$ComList[$key]['lasttime'] = ceil($time/3600)."小时前";
				}elseif($time>60 && $time<3600){
					$ComList[$key]['lasttime'] = ceil($time/60)."分钟前";
				}elseif($time<60){
					$ComList[$key]['lasttime'] = "1分钟前";
				}else{
					$ComList[$key]['lasttime'] = date("Y-m-d",$lasttime[$value]);
				}
    ...
    function _complie_joblist_start($tag_args)
    {
		...
		$time = time();
		$where = "`sdate`<'$time' and `edate`>'$time' and `state`='1'";
		if($config[sy_web_site]=="1")
		{
			if($_SESSION[cityid]>0 && $_SESSION[cityid]!="")
			{
		...
		if($paramer['jobids'])
		{
			$jobids=explode(",",$paramer['jobids']);
			foreach(explode(",",$paramer['jobids']) as $v)
			{
				if($job_type[$v]){
					$jobids[]=@implode(",",$job_type[$v]);
				}
			}
			$where.= " AND (`job1_son` IN (".@implode(',',$jobids).") OR `job_post` IN (".@implode(',',$jobids)."))";
		}
		if($paramer[jobin])
		{
			$where .= " AND (`job1` IN (".$paramer[jobin].") OR `job1_son` IN (".$paramer[jobpost].") OR `job_post` IN (".$paramer[jobpost]."))";
		}
		if($paramer[provinceid])
		{
			$where .= " AND `provinceid` = '".$paramer[provinceid]."'";
		...
		if($paramer[type])
		{
			if(is_array($paramer[type])){
				$where .= " AND `type` in (".@implode(",",$paramer[type]).")";
			}else{
		...
		if($paramer['orderby'])
		{
			$where.=" ORDER BY ".$paramer['orderby'];
		}else{
			$where.=$order.$sort;
		}
		if($paramer[where])
		...
		if($paramer[ispage])
		{
			$limit = $this->PageNav($paramer,$_GET,"company_job",$where,$Purl);
		...
		if(is_array($List))
		{
			...
			foreach($List as $key=>$value)
			{
				...
				$List[$key][description] = strip_tags($value[description]);
    ...
    function _complie_adlist_start($tag_args)
    {
		...
		include($path."/plus/ad_cache.php");
		if($paramer[adid]){
			...
			if(is_array($add_arr) && !empty($add_arr)){
				...
				foreach($add_arr as $key=>$value){
					...
					if(($value['did']==$_SESSION['did'] ||$value['did']=='0')&&$value['start']<time()&&$value['end']>time()){
						if($i>0 && $limit==$i){
							break;
						}
						if($length>0){
	...
	*/
	...
	function _complie_qclass_start($tag_args)
	{
		$paramer = $this->_parse_attrs($tag_args);
		$item = str_replace("'","",$paramer[item]);
		global $db,$db_config,$config;
		...
		$where .=" order by `sort`";
		$List = $db->select_all("q_class",$where.$limit);
		$ids=$db->DB_select_once("attention","`uid`='".$_COOKIE['uid']."' and `type`='2'","ids");
		$attention=@explode(",",$ids['ids']);
		if(is_array($List))
		{
			foreach($List as $key=>$value)
			{
				...
				if($paramer[intro])
				{
					$List[$key][intro] = mb_substr($value['intro'],0,$paramer[intro],"GBK");
				}
				$List[$key][class_url] = $this->Url("index","question",array("c"=>"topic","pid"=>$value[id]),"1");
				foreach($attention as $a_v)
				{
					if($a_v==$value['id'])
					{
						$List[$key]['is_atn']='1';//表示已经关注
					}
				}
			}
		}
