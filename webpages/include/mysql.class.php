...
class mysql {
	...
	function lt_array_action($job_info,$array=array())
	{
		if(!empty($array))
		{
			...
			include APP_PATH."/plus/com.cache.php";
			include APP_PATH."/plus/city.cache.php";
			include APP_PATH."/plus/lt.cache.php";
		...
		if($job_info["constitute"]!=""){
			$constitute = @explode(",",$job_info["constitute"]);
			foreach($constitute as $key=>$value){
				$constitute_info[]=$ltclass_name[$value];
			}
			$job_info["constitute_info"] = @implode(",",$constitute_info);
		}else{
			$job_info["constitute_info"] ="";
		}
		return $job_info;
	}
