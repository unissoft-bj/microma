...
class datacall{
	...
	public function resume($row){
		...
		if(@is_array($rows)){
			foreach($rows as $key=>$va){
				...
				if(strstr($row['code'],"{big_pic}"))$data[$key]['big_pic']=str_replace("./","/",$config['sy_weburl'].$va['resume_photo']);
				if(strstr($row['code'],"{small_pic}"))$data[$key]['small_pic']=str_replace("./","/",$config['sy_weburl'].$va['photo']);
				...
				if(strstr($row['code'],"{hj_city}"))$data[$key]['hj_city']=$city_name[$va['province']]."-".$city_name[$va['city']];
				if(strstr($row['code'],"{jzd_city}"))$data[$key]['jzd_city']=$city_name[$va['provinceid']]."-".$city_name[$va['cityid']];
	...
	public function lt_job($row){
		...
		$limit_num=$row['num']?$row['num']:10;
		$limit=" limit ".$limit_num;
		$field="a.id,a.uid,a.com_name,a.pr,a.hy,a.mun,a.desc,a.job_name,a.jobone,a.jobtwo,a.department,a.report,a.provinceid,a.cityid,a.edate,a.salary,a.constitute,a.social,a.live,a.years,a.desc,a.age,a.edu,a.sex,a.exp,a.full,a.language,a.eligible,a.qw_hy,a.lastupdate,b.realname";
		$rows = $this->obj->DB_select_alls("lt_job","lt_info","$where $order $limit",$field);
		if(is_array($rows)){
			foreach($rows as $key=>$va){
				if(strstr($row['code'],"{jobname}"))$data[$key]['jobname']=iconv_substr($va['job_name'],0,$row['titlelen'],$this->charset);
				if(strstr($row['code'],"{url}"))$data[$key]['url']=$views->lurl(array("url"=>"c:job_show,id:".$va['id']));
				if(strstr($row['code'],"{companyname}"))$data[$key]['companyname']=iconv_substr($va['com_name'],0,$row['titlelen'],$this->charset);
				if(strstr($row['code'],"{com_url}"))$data[$key]['com_url']=$views->curl(array("url"=>"id:".$va['uid']));
				...
				if(strstr($row['code'],"{years}"))$data[$key]['years']=$ltclass_name[$va['years']];
				if(strstr($row['code'],"{social}"))$data[$key]['social']=$ltclass_name[$va['social']];
				if(strstr($row['code'],"{live}"))$data[$key]['live']=$ltclass_name[$va['live']];
				...
				if(strstr($row['code'],"{job_desc}"))$data[$key]['desc']=iconv_substr($va['desc'],0,$row['infolen'],$this->charset);
				if(strstr($row['code'],"{salary}"))$data[$key]['salary']=$ltclass_name[$va['salary']];
				if(strstr($row['code'],"{edu}"))$data[$key]['edu']=$ltclass_name[$va['edu']];
				if(strstr($row['code'],"{sex}"))$data[$key]['sex']=$ltclass_name[$va['sex']];
		...
		if(is_array($rows)){
			foreach($rows as $key=>$va){
				...
				if($va['type']=='1'){
					...
					$url=$config['sy_weburl']."/lietou/index.php?c=service&keyword=".$va['key_name'];
				}elseif($va['type']=='7'){
					$url=$config['sy_weburl']."/lietou/index.php?c=post&keyword=".$va['key_name'];
				}
