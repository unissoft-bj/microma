...
class index_controller extends common{
	...
	function leftinfo_action($id)
	{
		...
		if($member['usertype']==1)
		{
			...
			include APP_PATH."/plus/lthy.cache.php";
			include APP_PATH."/plus/ltjob.cache.php";
			$leftinfo = $this->obj->DB_select_once("lt_info","`uid`='".$id."'");
			$leftinfo['typename'] = "猎头会员";
			$leftinfo['home'] = $city_name[$leftinfo['provinceid']] .$city_name[$leftinfo['cityid']];
		}
	...
	function reply_dynamic_action()
	{
		...
		$data['f_name']=iconv('utf-8', 'gbk', $_POST['f_name']);
		$data['status']=0;
		$data['ctime']=time();
		$data['uid']=$this->uid;
	...
	function applyfriend_action()
	{
		if($_GET['pass']){
			...
			$state_content = "与 <a href=\"".$this->config['sy_weburl']."/friend/index.php?c=profile&id=".$_GET['pass']."\" target=\"_blank\">".iconv('utf-8','gbk',$_GET['name'])."</a> 成为好友。";
			$sql['uid']=$this->uid;
	...
	function info_action()
	{
		...
		if($_POST['submitBtn'])
		{
			...
			$this->addstate($state_content);
 			$nid?$this->obj->ACT_layer_msg("更新成功！",9,$_SERVER['HTTP_REFERER']):$this->obj->ACT_layer_msg("更新失败！",8,$_SERVER['HTTP_REFERER']);
		}
		$this->yunset("class","4");
