...
class article_controller extends common
{
	...
	function show_action(){
		$_GET['id']=(int)$_GET['id'];
		$news=$this->obj->DB_select_alls("news_base","news_content","a.`id`='".$_GET['id']."' and a.`id`=b.`nbid`");
		$news_last=$this->obj->DB_select_once("news_base","`id`<'".$_GET['id']."' order by `id` desc");
 		...
 		if($news[0]["keyword"]!="")
		{
			...
			if(is_array($keyarr) && !empty($keyarr))
			{
				...
				$about=$this->obj->DB_select_all("news_base"," 1 AND  ($sqlkw) AND `id`!='".$_GET['id']."' order by `id` desc limit 6");
				if(is_array($about)){
