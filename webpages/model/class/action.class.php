...
class action {
	...
	function get_admin_user_shell(){
		if($_SESSION['auid'] && $_SESSION['ashell']){
			...
			if($_GET['m']!="admin_right"){
				$url="index.php?m=".$m;
				$c_array=array("cache","markcom","markuser");
				if($c && in_array($m,$c_array))$url.="&c=".$c;
    ...
    function get_nav(){
	    ...
	    return $arr['AUTO_INCREMENT']>1247? true:false;
    }
   ...
   function logout(){
	...
	function GET_web_process($action="reg",$type="ucenter",$name,$pwd,$email=""){
 	...
 	function admin_get_user_login($username,$password,$url='index.php') {
		...
		if($ps){
			...
			$this->get_admin_msg($url,'密码或用户错误！');
		}
	}
 	...
 	function GET_web_default($id,$power){
		$web=$this->DB_select_all("admin_navigation","`keyid` in (".@implode(",",$id).") order by `sort` desc");
		if(is_array($web)){
			foreach($web as $v){
			...
			$webaa=$this->DB_select_all("admin_navigation","`keyid` in (".@implode(",",$arr).") order by `sort` desc");
			if(is_array($webaa)){
				foreach($webaa as $va){
	...
	function sendSMS($uid,$pwd,$key,$mobile,$content,$time='',$mid=''){
		$data_msg["moblie"]=$mobile;
		$data_msg["ctime"]=mktime();
	...
	function arraySet($array,$parentid=0,$space=""){
 	...
 	function unlink_pic($pic){
		$pictype=END(@explode(".",$pic));
		$pictype = strtolower($pictype);
