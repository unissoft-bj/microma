	...
	function qqbind_action()
	{
		...
		if(($_GET['usertype']=='1' || $_GET['usertype']=='2') && $_SESSION['qq']['openid'])
		{
				...
				$this->add_cookie($userid,$username,$salt,$email,$pass,$usertype);
 				unset($_SESSION['qq']);
				$this->obj->ACT_msg($_SERVER['HTTP_REFERER'],"登录成功！",9);
		}
		$this->seo("qqlogin");
    ...
    function cert_action()
    {
		...
		if($id && is_array($arr) && $arr[0] && $arr[2]==$this->config['coding'])
		{
			...
			if(is_array($row))
			{
				...
				$id?$this->obj->ACT_msg($this->config['sy_weburl']."/member","认证成功！",9):$this->obj->ACT_msg($this->config['sy_weburl'],"认证失败，联系管理员认证！",8);
			}else{
