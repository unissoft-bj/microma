...
class admin_domain_controller extends common
{
		...
		if($_POST['add']){
			if($domain!="" && $title!="" ){
				...
				if(is_array($domain_list)){
					...
					$this->obj->ACT_layer_msg("域名绑定成功！",9,"index.php?m=admin_domain",2,1);
				}
			}else{
		...
		if($_POST['update']){
			if($domain!="" && $title!="" ){
				...
				if(is_array($domain_list)){
					...
					$this->DomainArr_action();
					$this->obj->ACT_layer_msg("域名绑定修改成功！",9,"index.php?m=admin_domain",2,1);
				}
			}else{
				$this->obj->ACT_layer_msg("信息填写不完整！",8,$_SERVER['HTTP_REFERER']);
			}
	...
	function DelDomain_action()
	{
		...
		if($_GET['delid'])
		{
			...
			$this->DomainArr_action();
			$this->layer_msg('域名删除成功！',9,0,"index.php?m=admin_domain");
		}
	}
