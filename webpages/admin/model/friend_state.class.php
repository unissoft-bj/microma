	...
	function del_action(){
		...
		if($_GET["time"]){
			...
			if($friend_state&&is_array($friend_state)){
				...
				if($ids&&is_array($ids)){
					$this->obj->DB_delete_all("friend_state","`id` in(".@implode(',',$ids).")","");
					$this->obj->DB_delete_all("friend_reply","`nid` in(".@implode(',',$ids).")","");
				}
			}
			$this->layer_msg('会员动态删除成功！',9,0,$_SERVER['HTTP_REFERER']);
		}
	}
}