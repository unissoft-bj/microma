...
class index_controller extends common{
		...
		if (!empty ($_POST["username"]) && !empty ($_POST["password"])){
			if(strstr($this->config["code_web"],'后台登陆')){
				if(md5($_POST["authcode"])===$_SESSION["authcode"]){
					...
					$this->obj->get_admin_msg("index.php","验证码错误");
				}
			}else{
				$this->obj->admin_get_user_login($_POST["username"], $_POST["password"]);
			}
	...
	function logout_action()
	{
		...
		$this->obj->get_admin_msg("index.php","您已成功退出！");
	}
	...
	function del_cache_action()
	{
		...
		if($cache==true)
		{
			$this->obj->get_admin_msg("index.php?m=admin_right","更新成功！");
		}else{
			$this->obj->get_admin_msg("index.php?m=admin_right","更新失败,请检查是否有权限！");
		}
	}
	function map_action(){
		$this->index_action(1);
