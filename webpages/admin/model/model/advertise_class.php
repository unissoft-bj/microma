...
class advertise
{
	...
	function model_ad_arr_action(){
		$show.="<?php\r\n\$ad_label='';\r\n";
		$ad_list = $this->obj->DB_select_all("ad","`is_open`='1' order by `id` desc");
		if(is_array($ad_list)){
			$time = time();
		...
		$show.="?>";
		$path = APP_PATH."/plus/ad_cache.php";
		$fp = @fopen($path,"w");
		@fwrite($fp,$show);
		@fclose($fp);
		@chmod($path,0777);
	...
	function model_saveadd_action($post,$pic=NULL)
	{
		...
		$value = "`ad_name`='$ad_name',`target`='$target',`time_start`='$time_start',`time_end`='$time_end',`ad_type`='$ad_type',`class_id`='$class_id',`is_check`='1',`did`='$did',`is_open`='$is_open'";
		if($ad_type=="word")
		{
			if($pic!="")
			{
	...
	{
		extract($post);
		$value = "`ad_name`='$ad_name',`target`='$target',`time_start`='$time_start',`time_end`='$time_end',`ad_type`='$ad_type',`class_id`='$class_id',`did`='$did',`is_open`='$is_open'";
		if($ad_type=="word")
		{
			if($pic!="")
			{
