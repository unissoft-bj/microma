...
class recharge_controller extends common{
		...
		if(isset($_POST['insert'])){
			...
			if(is_array($uidarr)){
				foreach($uidarr as $v){
					...
					$nid=$this->pay_member($v['uid'],$type,$num,$remark,$v['usertype'],$fs);
				}
			}
	...
	function pay_member($uid="",$type="price",$num,$remark,$usertype,$fs){
		...
		if($type=="price"){
			$_POST['type']=4;
			if($usertype==1){
				$nid=$this->obj->DB_update_all("member_statis",$pay_v,"`uid`='".$uid."'");
			}else{
