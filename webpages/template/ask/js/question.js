...
function reason(){
	...
	$.post(weburl+"/ask/index.php?c=q_repost",{reason:reason,eid:eid,type:type},function(data){
		if(data=='0'){
			...
			layer.msg('举报成功！', 2, 9,function(){$("#TB_window").hide();});return false;
		}else if(data=='2'){
			layer.msg('您已举报过该错误！', 2, 3,function(){$("#TB_window").hide();});return false;
		}else if(data=='3'){
			layer.msg('该错误已被他人举报！', 2, 3,function(){$("#TB_window").hide();});return false;
