...
function getresume(type,id){
	...
	$("#Add"+type).hide();
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=resume_ajax",{type:type,id:id},function(data){
		layer.closeAll();
		data=eval('('+data+')');
		if(type=="skill"){
...
function resume_del(table,id){
	...
	layer.confirm('确定要删除该项内容？', function(){
		layer.load('执行中，请稍候...',0);
		$.post("index.php?c=resume_del",{table:table,id:id,eid:eid},function(data){
			layer.closeAll();
			if(data!="0"){
				data=data.split("##");
				$("#"+table+id).remove();
				...
				$("#numresume").html(data[0]+"%");
				$(".play").attr("style","width:"+data[0]+"%");
				if(data[1]=="0"){
					$(".resume_"+table).hide();
				} 
				layer.msg('删除成功！', 2,9);				
...
function saveskill(){
	...
	/*if(skill==""){ 
		layer.msg('请选择技能类别！', 2, 2);
		return false;
	}
	if(ing==""){ 
		layer.msg('请选择熟练程度！', 2, 2);
		return false;
	}*/
	if(longtime==""){ 
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=skill",{skill:skill,ing:ing,name:name,longtime:longtime,eid:eid,id:id,table:"resume_skill",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#skill").hide();
			$("#Addskill").show();
			if(id>0){ 
				var html='<li><span>技能名称：</span>'+data.name+'</li><li><span>掌握时间：</span>'+data.longtime+'个月</li><li><span>技能类别：</span>'+data.skillval+'</li><li><span>熟练程度：</span>'+data.ingval+'</li>';
				$("#skill_"+id).html(html);
				layer.msg('操作成功！', 2,9);
				checkClose2('skill');	
				ScrollTo("skill_botton");
			}else{
				numresume(data.numresume,'skill');
				var html='<div class="expect_tj_list" id="skill'+data.id+'" onmousemove="movelook(\'skill\',\''+data.id+'\');" onmouseout="outlook(\'skill\',\''+data.id+'\');"><div class="expect_modify"><a href="javascript:getresume(\'skill\',\''+data.id+'\');">修改</a><a href="javascript:resume_del(\'skill\',\''+data.id+'\');">删除</a></div><ul class="expect_amend" id="skill_'+data.id+'"><li><span>技能名称：</span>'+data.name+'</li><li><span>掌握时间：</span>'+data.longtime+'个月</li><li><span>技能类别：</span>'+data.skillval+'</li><li><span>熟练程度：</span>'+data.ingval+'</li></ul></div>';
...
function savework(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=work",{sdate:sdate,edate:edate,name:name,department:department,eid:eid,title:title,content:content,id:id,table:"resume_work",submit:"1"},function(data){	
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#work").hide();
...
function saveproject(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=project",{sdate:sdate,edate:edate,name:name,sys:sys,eid:eid,title:title,content:content,id:id,table:"resume_project",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#project").hide();
...
function saveedu(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=edu",{sdate:sdate,edate:edate,name:name,specialty:specialty,eid:eid,title:title,content:content,id:id,table:"resume_edu",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#edu").hide();
...
function savetraining(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=training",{sdate:sdate,edate:edate,name:name,eid:eid,title:title,content:content,id:id,table:"resume_training",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#training").hide();
			...
			if(id>0){ 
				...
				layer.msg('操作成功！', 2,9);	
				checkClose2('training');
				ScrollTo("training_botton");
			}else{
...
function savecert(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=cert",{sdate:sdate,edate:edate,name:name,eid:eid,title:title,content:content,id:id,table:"resume_cert",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#cert").hide();
			$("#Addcert").show();
			if(id>0){ 
				var html='<li><span>证书全称：</span>'+data.name+'</li><li><span>有效时间：</span>'+data.sdate+'至'+data.edate+'</li><li><span>颁发单位：</span>'+data.title+'</li><li class="expect_amend_end"><span>证书描述：</span><em>'+data.content+'</em></li>';
				$("#cert_"+id).html(html);
				layer.msg('操作成功！', 2,9);
				checkClose2('cert');	
				ScrollTo("cert_botton");
			}else{
				numresume(data.numresume,'cert');
				var html='<div class="expect_tj_list" id="cert'+data.id+'" onmousemove="movelook(\'cert\',\''+data.id+'\');" onmouseout="outlook(\'cert\',\''+data.id+'\');"><div class="expect_modify"><a href="javascript:getresume(\'cert\',\''+data.id+'\');">修改</a><a href="javascript:resume_del(\'cert\',\''+data.id+'\');">删除</a></div><ul class="expect_amend" id="cert_'+data.id+'"><li><span>证书全称：</span>'+data.name+'</li><li><span>有效时间：</span>'+data.sdate+'至'+data.edate+'</li><li><span>颁发单位：</span>'+data.title+'</li><li class="expect_amend_end"><span>证书描述：</span><em>'+data.content+'</em></li></ul></div>';
...
function saveother(){
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=other",{eid:eid,title:title,content:content,id:id,table:"resume_other",submit:"1"},function(data){
		layer.closeAll();
		if(data!=0){
			data=eval('('+data+')');
			$("#other").hide();
...
function saveexpect(){	
	...
	layer.load('执行中，请稍候...',0);
	$.post("index.php?c=saveexpect",{name:name,hy:hy,job_classid:job_classid,provinceid:provinceid,cityid:cityid,three_cityid:three_cityid,salary:salary,type:type,report:report,eid:eid,submit:"1"},function(data){
		layer.closeAll();
		if(data==0){
			layer.msg('操作失败！', 2, 8);
		}else{
